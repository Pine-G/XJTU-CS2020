...
class CellularAutomaton inherits Board {
    ...
    evolve(): SELF_TYPE {
        (let position: Int <- 0, num: Int <- num_cells(), temp: String in {
    ...
    (*  This is where the background pattern is detremined by the user.  *)
    ...
    option(row: Int, col: Int): String {
        {
            ...
            (let i: Int <- 0, string: String <- "" in {
                while i < row loop {
                    ...
                    out_int(i);
                    out_string(">");
                    string <- string.concat(in_string().substr(0, col));
                    i <- i + 1;
                } pool;
                string;
            } );
...
class Main inherits CellularAutomaton {
    ...
    main(): SELF_TYPE {
        (let continue: Bool, choice: String in {
            ...
            while prompt2() loop {
                continue <- true;
                (let row: Int <- choose_row(), col: Int <- choose_col() in {
                    choice <- option(row, col);
                    cells <- (new CellularAutomaton).init(choice, row, col);
                    cells.print();
                } );
                while continue loop
                    if prompt() then { cells.evolve(); cells.print(); }
