...
int main(int argc, char** argv) {
	...
	while (optind < argc) {
	    ...
	    optind++;
	    std::cout << std::endl;
	    if (pare1 > 0) std::cout << "Missing " << pare1 << " )" << std::endl;
	    if (pare2 > 0) std::cout << "Missing " << pare2 << " }" << std::endl;
	    if (if_fi > 0) std::cout << "Missing " << if_fi << " FI" << std::endl;
	    if (loop_pool > 0) std::cout << "Missing " << loop_pool << " POOL" << std::endl;
	    if (case_esac > 0) std::cout << "Missing " << case_esac << " ESAC" << std::endl;
	}
	exit(0);
}
